...
export const addPost = (postData) => dispatch => {
  axios
    .post("/api/drafts/add", postData) //add route endpoint we made in server.js
    .then(res => {
...
export const showPosts = () => dispatch => {
  axios
    .get("/api/drafts/add") //get route endpoint we made in server.js
    .then(res => {
...
export const deletePost = (postId) => dispatch => {
  axios
    .delete(`/api/drafts${postId}`) //delete route endpoint we made in server.js
    .then(res => {
      dispatch({ type: NOTIFICATION, payload: res.data });
      setTimeout(() =>
        dispatch({ type: NOTIFICATION, payload: {} })
